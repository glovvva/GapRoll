...
async def get_departments(
    ...
    """
    Zwraca listę unikalnych działów z payroll_data (z raw_data.department lub kolumny department).
    """
    try:
        ...
        for row in (result.data or []):
            ...
            dept = raw.get("department") or row.get("department")
        ...
        logger.error("get_departments: %s", e)
...
async def get_pay_gap_analysis(
    ...
    try:
        ...
        response = (
            supabase.table("payroll_data")
            .select("first_name, last_name, position, gender, salary, raw_data")
            .eq("user_id", user_id)
            .execute()
        )
        ...
        if department and department != "all":
            data = [
                r
                for r in data
                if (r.get("raw_data") or {}).get("department") == department
                or r.get("department") == department
            ]
            if not data:
                raise HTTPException(
                    status_code=404,
                    detail=f"Brak danych dla działu: {department}",
                )
        ...
        logger.error("get_pay_gap_analysis: %s", e)
        raise HTTPException(status_code=500, detail=str(e)) from e
...
class EVGScoreRequest(BaseModel):
    ...
    positions: List[str] = Field(..., max_length=500)

    @field_validator("positions", mode="after")
    @classmethod
    def position_length(cls, v: List[str]) -> List[str]:
        for i, p in enumerate(v):
            if len(p) > 200:
                raise ValueError(f"Position at index {i} exceeds max length 200")
        return v
...
{{
  ...
  "reasoning": "Stanowisko {position} wymaga [jedno zdanie - formalna polszczyzna]."
    ...
    try:
        ...
        logger.error("scoring position failed: %s", e)
        raise HTTPException(
            status_code=500,
...
async def score_positions(
    ...
    try:
        ...
        key = settings.SUPABASE_SERVICE_ROLE_KEY or settings.SUPABASE_KEY
        supabase = get_supabase_client(settings.SUPABASE_URL, key)
        # One query for all cached job_valuations for this user (avoid N+1)
        cached_rows: Dict[str, Dict[str, Any]] = {}
        try:
            all_cached = (
                supabase.table("job_valuations")
                .select("position, evg_score, skills, effort, responsibility, conditions, reasoning")
                .eq("user_id", user_id)
                .execute()
            )
            if all_cached.data:
                for row in all_cached.data:
                    cached_rows[row["position"]] = row
        except Exception:
            pass

        results = []
        for position in body.positions:
            if position in cached_rows:
                results.append(cached_rows[position])
                continue
        ...
        logger.error("score_positions: %s", e)
        raise HTTPException(status_code=500, detail=str(e)) from e
...
async def clear_evg_cache(
    ...
    try:
        ...
        logger.error("clear_evg_cache: %s", e)
        raise HTTPException(
            status_code=500,
...
async def get_dashboard_metrics(
    ...
    if cached and now < cached[0]:
        logger.debug("timing /analysis/dashboard-metrics (cache): %.3fs", time.time() - t0)
        return cached[1]
    ...
    try:
        ...
        logger.error("get_dashboard_metrics fetch: %s", e)
        raise HTTPException(status_code=500, detail=str(e)) from e
    ...
    logger.debug("timing /analysis/dashboard-metrics: %.3fs", time.time() - t0)
    return out
...
async def get_art16_analysis(
    ...
    try:
        ...
        response = (
            supabase.table("payroll_data")
            .select("position, gender, salary, raw_data")
            .eq("user_id", user_id)
            .execute()
        )
        ...
        logger.debug("timing /analysis/art16: %.3fs", time.time() - t0)
        return {
            "quartiles": quartiles,
        ...
        logger.error("get_art16_analysis: %s", e)
        raise HTTPException(status_code=500, detail=str(e)) from e
