...
async def get_current_partner(
    ...
    try:
        ...
        logger.error("get_current_partner: %s", e)
...
class OnboardClientRequest(BaseModel):
    ...
    email: EmailStr
    employee_count: int = Field(..., ge=50, le=100000)
    tier: str = Field(..., pattern="^(compliance|strategia)$", max_length=50)
...
async def onboard_client(
    ...
    try:
        ...
        logger.error("subscriptions insert: %s", e)
    ...
    try:
        await log_audit_event(
            ...
            metadata={"nip": body.nip, "tier": body.tier},
            request=request,
        )
    except Exception:
        pass
...
async def get_mrr(
    ...
    try:
        ...
        logger.error("get_mrr companies: %s", e)
