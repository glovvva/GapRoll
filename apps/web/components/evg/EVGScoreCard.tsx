...
interface EVGScoreCardProps {
  ...
  /** HITL: zatwierdzenie pojedynczego stanowiska */
  onApprove?: (positionId: string) => void;
  isApproved?: boolean;
}
...
export function EVGScoreCard({ position, axes, onOverride, onApprove, isApproved }: EVGScoreCardProps) {
        ...
        <div className="flex flex-col gap-2">
          ...
          {onApprove && (
            <Button
              ...
              onClick={() => onApprove(position.id)}
              className={isApproved ? "text-green-400 border-green-700" : "text-slate-400"}
            >
              {isApproved ? "✓ Zatwierdzone" : "Zatwierdź"}
            </Button>
          )}
        </div>
      </CardContent>
