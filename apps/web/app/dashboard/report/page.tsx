...
import { Art16ExportButton } from "@/components/reports/Art16ExportButton";
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function ReportPage() {
  ...
  const [evgApproved, setEvgApproved] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    async function fetchEvgApproval() {
      try {
        const res = await fetchWithAuthCached("/api/evg/approval-status");
        if (res.ok) {
          const json = (await res.json()) as { approved?: boolean };
          setEvgApproved(json.approved === true);
        } else {
          setEvgApproved(false);
        }
      } catch {
        setEvgApproved(false);
      }
    ...
    try {
      ...
      const url =
        selectedDepartment && selectedDepartment !== "all"
          ? `/api/analysis/art16?department=${encodeURIComponent(selectedDepartment)}`
          : "/api/analysis/art16";
  ...
  return (
    <div className={sectionSpacing}>
      {/* Soft warning gdy EVG nie zatwierdzone */}
      {evgApproved === false && (
        <div className="mb-4 p-3 bg-amber-900/30 border border-amber-700 rounded-lg text-sm text-amber-300">
          ⚠ Wartościowanie EVG nie zostało jeszcze zatwierdzone przez człowieka.{" "}
          <a href="/dashboard/evg" className="underline ml-1">
            Przejdź do EVG Scoring →
          </a>
        </div>
      )}
      {/* SECTION 1: HEADER */}
